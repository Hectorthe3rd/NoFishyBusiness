...
from backend.assistant import get_assistant_reply, stream_assistant_reply  # noqa: E402
...
@app.post("/assistant/stream", summary="AI Assistant (streaming)")
def assistant_stream(request: AssistantRequest):
    """Stream the assistant reply token-by-token using Server-Sent Events.

    Returns a text/plain streaming response. Each chunk is a raw text
    fragment of the reply as it is generated by the LLM. The frontend
    consumes this with requests(stream=True) and renders chunks in real time.
    """
    return StreamingResponse(
        stream_assistant_reply(request.message, request.history),
